...
class CalibrationApiService {
  async getCalibrationConfig(): Promise<CalibrationConfig> {
    try {
      const response = await visionApi.get<any>('/api/v1/inference/calibration')
  ...
  async saveCalibrationConfig(payload: CalibrationRequestPayload): Promise<CalibrationConfig> {
    try {
      const response = await visionApi.post<any>('/api/v1/inference/calibration', {
  ...
  async clearCalibrationConfig(): Promise<CalibrationConfig> {
    try {
      const response = await visionApi.delete<any>('/api/v1/inference/calibration')
      return this.transformResponse(response)
    } catch (error) {
