  ...
  const renderCalibrationOverlay = () => {
    ...
    return (
      <div className="absolute inset-0 pointer-events-none">
        ...
        {/* เส้น grid แนวตั้ง (12 คอลัมน์ = 13 เส้น) */}
        ...
        {isCalibrationMode && (
          <>
            ...
            {/* จุดควบคุมเส้นแนวตั้ง (ลากได้ทุกเส้น รวมขอบ) */}
            {displayColumns.map((x, index) => (
              <button
            ...
            {/* จุดควบคุมเส้นแนวนอน (ลากได้ทุกเส้น รวมขอบ) */}
            {displayRows.map((y, index) => (
              <button
          ...
          <>
            ...
            >
              <img 
                ref={imageRef}
                src={displayImageSrc} 
                alt={annotatedImageUrl && displayImageSrc === annotatedImageUrl ? "Annotated Result" : "Original Image"} 
                className="w-full h-full object-contain pointer-events-none"
                onLoad={(event) => {
                  const currentSrc = displayImageSrc;
                  logger.debug('ImageUpload: Image loaded successfully:', currentSrc);
                  logger.debug('ImageUpload: Is annotated image?', !!annotatedImageUrl && annotatedImageUrl === currentSrc);
